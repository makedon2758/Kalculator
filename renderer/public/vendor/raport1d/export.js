...
// ---- ПУБЛИЧНО: интерфейс для main.js ----
export function buildExportData(result, sheetW = 1000, opts = {}) {
  const { gridView, anti } = opts || {};
  if (result && Array.isArray(result.byTyp)) {
    return __buildGroupedExport(result.byTyp, sheetW, opts);
  }
...
export async function exportXLSX(plan, leftovers) {
  ...
  XLSX.writeFile(wb, 'raport_rozkroj.xlsx', { compression: true });
}
...
export async function exportReport(_fmt, plan, leftovers) {
  return exportXLSX(plan, leftovers);
}
